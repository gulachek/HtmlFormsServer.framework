...
- (nonnull instancetype)initWithPort:(NSInteger)port
                          sessionDir:(NSURL *)sessionDir {
  unsigned short shortPort = port;
  NSString *sessionDirStr = [sessionDir path];
  const char *sessionDirCstr =
      [sessionDirStr cStringUsingEncoding:NSUTF8StringEncoding];
...
- (void)closeWindow:(NSInteger)windowId {
  html_forms_server_close_window(self->server_, windowId);
}

- (void)start {
  if (self->is_running_) {
    os_log_debug(self->log_,
                 "Skipping server start because it is already running");
    return;
  }

  os_log_info(self->log_, "Starting server");
  self->is_running_ = YES;
  html_forms_server_set_event_callback(self->server_, evt_callback,
                                       (__bridge void *)self);
...
- (void)stop {
  if (!self->is_running_) {
    os_log_debug(self->log_,
                 "Skipping server stop because it is already stopped");
    return;
  }
...
- (void)handleEvent:(const html_forms_server_event *)evt {
  ...
  if (evt->type == HTML_FORMS_SERVER_EVENT_OPEN_URL) {
    ...
    NSString *urlStr = [NSString stringWithCString:evt->data.open_url.url
                                          encoding:NSUTF8StringEncoding];
    NSURL *url = [NSURL URLWithString:urlStr];

    if (os_log_debug_enabled(self->log_)) {
      os_log_debug(self->log_, "Navigating window %ld to url=%@", (long)winId,
                   url);
    }
    ...
    NSString *errMsg = [NSString stringWithCString:evt->data.show_err.msg
                                          encoding:NSUTF8StringEncoding];

    if (os_log_debug_enabled(self->log_)) {
      os_log_debug(self->log_, "Showing error '%@' on window %ld", errMsg,
                   (long)winId);
    }
    ...
    os_log(self->log_, "Received server event with unknown type (type=%d)",
           evt->type);
  }
